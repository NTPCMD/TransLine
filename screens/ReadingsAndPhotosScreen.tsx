...
interface ReadingsAndPhotosScreenProps {
  onContinue: (
    readings: { odometer: string; manual: string },
    photos: { odometer: string; manual: string }
  ) => void;
  onBack: () => void;
}
...
export function ReadingsAndPhotosScreen({ onContinue, onBack }: ReadingsAndPhotosScreenProps) {
  ...
  const [errors, setErrors] = useState<{ odometer: string; photo: string }>({ odometer: '', photo: '' });
  ...
  const odometerValue = parseFloat(odometerReading);
  const isOdometerValid = !isNaN(odometerValue) && odometerValue > 0;
  const hasOdometerPhoto = Boolean(odometerPhoto);

  const validate = () => {
    const newErrors = { odometer: '', photo: '' };

    if (!odometerReading) {
      newErrors.odometer = 'Odometer reading is required.';
    } else if (!isOdometerValid) {
      newErrors.odometer = 'Enter an odometer value greater than zero.';
    }

    if (!hasOdometerPhoto) {
      newErrors.photo = 'Odometer photo is required.';
    }

    setErrors(newErrors);
    return !newErrors.odometer && !newErrors.photo;
  };
  ...
  const handleSubmit = () => {
    ...
    onContinue(
      { odometer: odometerReading, manual: manualReading },
      { odometer: odometerPhoto, manual: manualPhoto }
    );
  };
  ...
  const canSubmit = isOdometerValid && hasOdometerPhoto;

  return (
    <div className="h-full flex flex-col bg-white">
      <ScreenHeader title="Readings & Photos" onBack={onBack} />
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-3">
            ...
            {errors.odometer && (
              <p className="text-sm text-[#D32F2F]">{errors.odometer}</p>
            )}
            ...
            {errors.photo && (
              <p className="text-sm text-[#D32F2F]">{errors.photo}</p>
            )}
          </div>

          <div className="h-px bg-[#9E9E9E]" />

          {/* Manual Reading Section */}
          <div className="flex flex-col gap-3">
            <div>
              <h3>Manual Reading</h3>
              <p className="text-sm text-[#9E9E9E] mt-1">(Optional)</p>
            </div>
            
            <input
              type="number"
              placeholder="Enter manual reading"
              value={manualReading}
            ...
            {manualPhoto ? (
              <div className="relative bg-[#F2F2F2] rounded-[12px] overflow-hidden">
                <img src={manualPhoto} alt="Manual Reading" className="w-full h-auto" />
                <div className="absolute top-3 left-3 bg-black bg-opacity-60 text-white px-3 py-2 rounded-lg text-sm">
                  <div className="flex items-center gap-2 mb-1">
                    <Clock size={14} />
                    {getCurrentDateTime()}
            ...
            <div className="flex gap-2">
              ...
              >
                <div className="flex items-center justify-center gap-2">
                  <Camera size={18} />
                  {manualPhoto ? 'Retake Photo' : 'Capture Photo'}
                </div>
              </Button>
              <Button
                variant="outline"
